...
# pgrep -fc prints a 0 and ALSO exits non-zero when nothing matches, so an
# "|| echo 0" fallback appends a second zero and the value becomes two lines.
# Documented in this project twice already; do not reintroduce it.
echo "before:"
printf '  clang %s   siso %s\n' "$(pgrep -fc clang 2>/dev/null)" "$(pgrep -fc siso 2>/dev/null)"
